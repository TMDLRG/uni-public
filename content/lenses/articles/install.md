...
agrees with itself. Every version on the page is quoted from the file that declares it, and where two
...
scripting parts, an interpreter for the experiment pipeline, an older platform version for the game,
and a set of paid or platform-specific things for the studio.

It gives install steps for each part in turn, tells you to change a default password that ships in a
setup script before anything else, and ends with a plain list of what you will not be able to install
at all, because the source repositories are not public and some of the data is not redistributed.
...
and the disagreement is named rather than smoothed over — because a list that resolves conflicts
silently will send you to the wrong version and give you no way to find out why.
...
one; a media server; a media converter on the path; a certificate tool; a shell; and a container
...
tells you to change a default remote-console password that ships in the setup script, noting that a
...
network does not exist for you, several absolute paths are hard-coded, the observed-experiment
