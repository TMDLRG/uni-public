...
lens_schema: 1
scope: wiki
key: flagellum/cross-study-parity
corpus: flagellum
source_sha256: 2f388f58f1435338
source_body_sha256: 2f388f58f1435338
source_title: Cross-study biological parity program
source_words: 1895
...
This page reports what happened when the project stopped testing itself against one published
dataset and started testing itself against a corpus of them.

The headline is a refusal. The model now expresses much more of the observed biology, and full
parity is still false. There is a fixed list of gates, and every one of them has to pass; a good
average cannot make up for a missing required area, and the page says so directly.

The best new positive result is not one universal equation. It is a model built from separate
modules, each with its own declared biology, units and evidence, that can be traced back to
particular experiments. The best new negative result is that one exact lattice calculation gives
two incompatible answers depending on which summary of the same data it is fitted to.

The page also refuses to inflate its sample. Hundreds of thousands of time samples are counted
as a few hundred motors and cells, because repeated measurements of one motor are still one
experimental unit.

<!--CLEAR-->

The verdict comes first. The expanded model expresses substantially more observed biology, and
...
Additional couplings are reported with their effect sizes and their required controls, and with
the statement that they support association rather than an unrestricted causal chain.

The gate table gives each gate a status and one line on what the result establishes. Where a
gate is not established, the reason is that no unit-safe parameter frozen in one laboratory has
yet predicted commensurate raw observations from another, or that no intervention discriminates
the project's account from matched alternatives. Where a gate is blocked, the reason is
external.

A short section draws the boundary for the physical export. Two conflicts remain: the lattice is
an assay-specific statistical model rather than established molecular geometry, and the dwell
states are phenomenological until a separate molecular measurement identifies them. So the
export is a transparent mathematical model, not a literal molecular replica of the motor.

The page ends with a numbered list of what a complete model still requires, and with the line
that gives the list its weight: no software-only action can honestly complete the last three.
They remain experimental contracts, not simulated passes.
