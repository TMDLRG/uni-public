...
lens_schema: 1
scope: wiki
key: cookbook/recipes-natura-cn-06-sperm
corpus: cookbook
source_sha256: 79b4993c8d5ff2b0
source_body_sha256: 79b4993c8d5ff2b0
source_title: CN-06 — Sperm: the minimal motile delivery vehicle
source_words: 7811
...
<!--CLEAR-->
This chapter treats a single motile cell as an engineering problem with an unforgiving specification. The vehicle is finished at launch — no transcription, no repair, no rebuild — so whatever machinery it shipped with is all it will ever have, even though fuel can be drawn from the medium along the way. The environment is viscous enough that inertia is irrelevant.

That regime is the chapter's spine, and it insists you do the arithmetic before drawing anything. At this scale a shape sequence that simply retraces itself produces no net movement at all, which rules out whole families of design. The falsifier is written out with unusual care: exhibit one swimmer achieving sustained progress in an ordinary fluid using a strictly reversible stroke, with the fluid's behaviour measured and the obvious loopholes excluded — and the chapter names in advance the escape that does not count, because a springy fluid is already inside the theory rather than a refutation of it.

From there it works through the machine. The motor slides rather than bends, and the chapter is precise about that distinction. A section is given entirely to the principle that a rate without its condition is not a number, and the receipt is two very different beat frequencies measured from the same cells at the same temperature. Efficiency is reported as poor and treated as instructive rather than embarrassing. Where the energy actually comes from turns out to have been genuinely contested, and the chapter carries the contest rather than settling it — an experiment that disabled one pathway produced immobile cells with the other pathway intact, which overturned the obvious anatomical inference in one animal and leaves the human case unsettled, so neither may be asserted.

Navigation is read as inference, with the act of moving to sample a gradient treated as an epistemic action — a phrase the chapter fences the moment it uses it, because it does not claim that a sperm decides, wants, seeks or knows anything in any sense involving experience. The description is statistical: a policy weighted partly by what it would resolve, and a helix that is a sampling geometry. The chapter then examines the folk explanation of why such a cell samples over time rather than across its body, and separates three claims with three different statuses — the mechanism actually reported, a modelling argument that reaches the same conclusion for a different reason, and the popular version, which is recorded as unearned in the form it is usually given.

The recorded negatives are careful about their own wording. A hypothesis about a specialised non-fertilising caste made a testable prediction; the test was run and the prediction did not reproduce, and the chapter insists on matching the source exactly, noting that the authors hedged to unlikely rather than refuted, and that some signal was seen but none of it in the predicted pattern. It says this bullet exists to celebrate honesty about a negative result, which is why the wording must be exact. A racing narrative is marked inadmissible because nothing in it could be refuted, and the competing accounts are printed and left unresolved. Several borrowed constants are flagged as belonging to a different species, differently prepared, at a stated concentration and load — all four conditions being easy to drop.
