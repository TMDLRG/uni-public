...
<!--CLEAR-->
This chapter treats a single motile cell as an engineering problem with an unforgiving specification. None of it was measured here. The numbers come from other people's laboratories, cited, so this is not one of the project's own results. The vehicle is finished at launch — no transcription, no repair, no rebuild — so whatever machinery it shipped with is all it will ever have, even though fuel can be drawn from the medium along the way. The environment is viscous enough that inertia is irrelevant.

That regime is the chapter's spine, and it insists you do the arithmetic before drawing anything. At this scale a shape sequence that simply retraces itself produces no net movement at all, which rules out whole families of design. The falsifier is written out with unusual care. Exhibit one swimmer achieving sustained progress in an ordinary fluid using a strictly reversible stroke, with the fluid's behaviour measured and the obvious loopholes excluded. The chapter names in advance the escape that does not count, because a springy fluid is already inside the theory rather than a refutation of it.
...
Navigation is read as inference, with the act of moving to sample a gradient treated as an epistemic action. The chapter fences that phrase the moment it uses it: it does not claim a sperm decides, wants, seeks or knows anything in any sense involving experience. The description is statistical: a policy weighted partly by what it would resolve, and a helix that is a sampling geometry. The chapter then examines the folk explanation of why such a cell samples over time rather than across its body. It separates three claims with three different statuses: the mechanism actually reported, a modelling argument that reaches the same conclusion for a different reason, and the popular version, which is recorded as unearned in the form it is usually given.
