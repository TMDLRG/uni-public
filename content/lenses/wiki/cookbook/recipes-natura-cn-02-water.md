...
lens_schema: 1
scope: wiki
key: cookbook/recipes-natura-cn-02-water
corpus: cookbook
source_sha256: 1cfac2f80bbdb263
source_body_sha256: 1cfac2f80bbdb263
source_title: CN-02 — Water: the anomalous solvent everything else assumes
source_words: 6257
...
---
<!--PLAIN-->
Water gets a spec sheet here, as the medium every other nature chapter quietly assumes. Everything in it is cited from chemistry, so it contributes no evidence to the programme's own results — and it closes with a list of what is not claimed, starting with any suggestion that water is fine-tuned, purposive, or evidence of anything beyond chemistry.
...
<!--CLEAR-->
This chapter treats water as a component with a specification rather than as scenery. Its structural claim is that water's biologically load-bearing behaviour follows from the shape of its hydrogen-bonded network, and that in each case the right thermodynamic account is about the water rather than about whatever is dissolved in it. It is fenced as modelled: individual rows carry their own classes, but the chapter as a whole composes measured constants through stated assumptions.
...
The falsifier is stated structurally rather than row by row: exhibit a liquid with the same network that lacks the package of anomalies, or one without the network that has them all, or overturn the thermodynamic account of the assembly effect at small scales. Individual numbers falling outside their stated scope move only their own row.

The recorded negatives are the most instructive part, and the chapter says the receipt is the lesson rather than the verdict. A celebrated claim about water retaining a memory of what had been diluted out of it was re-run in the same laboratory with the same people, and the single change that mattered was blinding: with the sample codes sealed, the effect vanished. The chapter draws the careful conclusion — the effect was located in the observer, and the people who found this out did so by running the experiment rather than by arguing. A second claim, about intention changing water, is handled more subtly, because a controlled study exists and reported a positive result; the chapter shows that what it measured was a subjective aesthetic rating of selected photographs, with no physical property of water measured at all, and with the claimant among the authors. A third is split three ways: an interfacial phenomenon that is real and independently replicated, an explanation of it that is contested, and a health claim that has no measurement behind it at all.
