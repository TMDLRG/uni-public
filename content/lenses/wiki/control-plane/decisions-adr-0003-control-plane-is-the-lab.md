...
The picture it describes is spatial. You author a judgement by standing at the thing you are ruling on. A run under way appears mid-room at a size you could walk around, with its pre-registered gate and its falsifier beside it, before there is an answer. Rooms have airlocks that need two keys. Projects are doorways into their own worlds. The signal-reporting part is the sky: always in view, never enterable.
...
The design is described in spatial terms and each spatial idea carries a meaning. You author a verdict by standing at the thing you are ruling on. The run under way renders mid-room at a scale you could walk around, with its pre-registered gate and falsifier beside it, before the run has an answer. Rooms are volumes with two-key airlocks. Projects are portals: you look through to a world's own view, or step through to work in it, and a portal never re-derives that world's state or reimplements its renderer. The signal-reporting body is the sky — always in view, never enterable.

The consequences are stated both ways. In favour: there is no split between the tool that decides and the screen that shows deciding, so no state has to be kept in step between them, and a room that will not open is itself the refusal, needing no dialog. Against: a rendered surface is heavier to build and test than a form, which is why screenshot-based acceptance tests are required; the record says the risk is reduced by borrowing an existing technique rather than inventing one. Neutrally, the lab is watched like anything else and gets no privileged view of itself.

Two alternatives were rejected by the operator — a dashboard, and reusing the existing god-view — and a third, making the lab view a separate body, was rejected because it would put the room and its command path on opposite sides of a network boundary.

The falsifier: if the lab view can display a state the command path did not produce, or if authoring a verdict requires leaving the room, the decision has been violated.
