...
The most useful section lists what must not be softened. An outside witness, compromised. An anchor that is not unforgeable. A contract proposed rather than adopted. And a set of gates with no rows in the ledger that counts — the list of results nobody may edit. The last section keeps, rather than deletes, everything this file used to say while it was false.
...
A table shows where each stage stands: several done, one blocked on the operator, one in progress awaiting a check only a person can make, and the last two planned. One stage is deliberately last, with the reason given — editing a document to match a broken world closes the only signal telling the truth.

The measured-state section is a series of generated blocks, each with a note about the error it exists to prevent. The gate counts are stated in two forms, because both had been written before without saying which was which. The tally for the ledger, only ever added to, is given per name and says so, because a count whose derivation is unstated is how a backlog and its history came to be reported as one word.

Then comes a block that answers nothing and lists commands instead. Three things are deliberately not stated, because no committed file can hold them honestly: they are facts about a run or about now, not about the tree. The page says this section used to answer all three, and that one answer was false again within minutes while committed as present tense.
...
A section headed what must not be softened lists the standing adverse facts: the outside witness is compromised and the anchor is tamper-evident rather than unforgeable. Every registered gate has no rows in the ledger that counts, though the schema says it must. No verdict has yet been authored about a real scientific claim. And a guard binds this codebase's paths, not the machine.
...
The final section keeps everything the file used to say while it was false, clause by clause, and explains why. This file has no gate, so it drifts silently, and the only defence is a reader who checks the numbers against the world.
