...
lens_schema: 1
scope: wiki
key: minecraft/reports-lab-uat-checklist
corpus: minecraft
source_sha256: 89d45b6923bdd3fc
source_body_sha256: 89d45b6923bdd3fc
source_title: SP.Lab — User Acceptance Test (UAT) checklist
source_words: 625
...
The table itself covers a wide range: consistency of units, reproducing surface gravity for several bodies from a standard formula, showing that gravity persists at near-vacuum pressure on two airless bodies, two cases where a rival pressure-based account overshoots badly, the direction of an effect linking an absorber to surface radiation, the case of no absorber at all, two bioenergetic monotonicity checks, the collapse of viability when a membrane fails, and the difference between a cell that requires oxygen and one that does not. Further rows cover a model comparison penalising a circular formula, an audit that flags a hidden scaling factor, a thermal case separating one loss mechanism from another, a user interface requirement that every claim carry an evidence class, and a vocabulary check that no module reports an unsupported claim using a forbidden word.
