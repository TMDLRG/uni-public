...
<!--CLEAR-->
This is the operator's run sheet, written to be printed and used while running a live show. Its opening line says everything on it was tried live rather than asserted, that where something is unproven it says so, and it names the recorded runs behind it. It starts with one call that answers what state everything is in, and a rule: never go searching through files to answer a state question. It also points at a glance surface and notes that it is a native widget rather than a page.

The next section lists what only the human may do, and each row gives the reason. Typing the keys, because an agent must never handle one. Choosing the camera hardware. Unmuting the microphone, because everything boots muted by policy. Filling in the show metadata. Typing the word that puts the show on air. And ruling on one contested limit nobody else can decide.
...
An alarm section explains an off-monitor warning that stays silent unless you are measurably on air, and names the three faults that trip it. It is careful about its limits: acknowledging it silences the sound for a while, does not clear the banner, and never disarms anything.
...
Going live is three steps, with measured guidance on which scenes actually render and a warning not to rebuild the stage first. Two limits follow. One is a contested reading the operator must rule on, stated as display-only with no code enforcing it. The other bans a specific phrase from the air, because the number behind it has one source and no corroboration.

The staying-live section is the bluntest. A long run is stated as not underwritten, and a table of risks follows. It includes a documented picture failure whose automatic recovery does not exist, and a crash that drops all outward air and needs the operator's code to restore. It also includes two supervisors that disagree while on air, and the absence of any soak gate.
