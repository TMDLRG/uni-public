...
One fence is stated bluntly. The organ's signal is functional only. It is never described as something felt.
...
The required checks then ask every new organ to name its factor, its emptying and filling transitions with reachability asserted, its comfortable-level preference, and the strength of its prior; to build that preference from a declared map fixed before any policy is evaluated; to ship the clone test in the same change; and to register two gates in advance, one for anticipatory regulation and one for a cycle rather than a flatline or a runaway climb. The last check is the claim fence: the signal is functional access only, never surfaced as felt.
