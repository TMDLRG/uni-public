...
---
<!--PLAIN-->
This brief specifies how evidence will be gathered for a paired experiment, and it is careful to say what it does not do: it changes no code, deploys nothing, and makes no verdict of its own. It specifies how to gather the records of what was run that an argument will later be made from.
...
The final section is operating notes and traps, each cited. The port confusion comes up again, along with a server setting that must already be in place or agents will drop. A block identifier must match the species actually present, or a counter silently stays at zero. An assumption about one command being non-destructive must be checked once before it is trusted in a loop. And on a lost connection the instruction is to reconnect rather than crash, writing a gap row so the cadence is preserved.
