...
A knowledge section names the shapes involved: the per-factor pieces of the model and which is which, the convention for which way round the matrices are stored, and the discipline that only summary values cross a level boundary rather than any live belief structure. It also names the kind of typed specification that must sit beside the code, covering the state space, the observation channels, the action space, the preferences, the policies, the learning parameters, the precision schedule, the validation anchors and the claim fence. It names the kinds of property test that count as evidence, and points at existing modules and tests as templates.
