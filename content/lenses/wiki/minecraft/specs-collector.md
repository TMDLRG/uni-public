...
A binding limit opens the page: none of the values it logs is a felt state, and none of it carries weight for awareness or life.
...
The rest is schema and procedure. It gives the exact shape of a data line, and the shape of a heartbeat line, which is written unconditionally. It gives a rule for reconciling rows across collectors, and a readiness checklist the collector must satisfy before an experiment can start. And it gives deployment and approval steps that are explicitly not to be run automatically, with an index of the files that make up the artifact.
