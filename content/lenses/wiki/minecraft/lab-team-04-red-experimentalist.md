...
<!--CLEAR-->
This is a role description for one member of an adversarial review team. It speaks after the mathematics and the implementation have survived, and its job is to design the paired, pre-registered test that would falsify the behavioural claim before the complexity of a real world hides it.
...
The required checks then fix the shape of an acceptable test: pre-registered in a linked document, paired with a matched control, collecting continuous measurements at a stated cadence in a way that survives an assistant losing its context, with a success condition that requires all of several things at once and a named refuting condition, minimum numbers per arm for both live and offline claims, independent confirmation of behaviour and of mechanism, and a verdict recorded in the same document as the gate with enough detail to reproduce every number cited.
