...
Nine sections follow, covering the state space, the channels, the actions, the preferences, the policy, the learning parameters, the precision schedule, the anchors that check it, and a limit on what may be claimed.
...
<!--CLEAR-->
This document does double duty. It is the typed specification of a term that is already merged and live behind a switch that is off by default. It is also the template that every later organ specification must reproduce, in the same nine sections and the same order, citing real source locations and carrying a limit on what may be claimed.
...
The source files are listed with the exact lines that implement each piece. That covers the term itself, its integration into the real deep-planning decider, its mirror in the shallower path, and the default value of its coupling. It also covers the places where the inheritable setting is defaulted, back-filled, read defensively, and drawn during mutation and recombination.
