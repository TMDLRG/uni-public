...
The first section lists the knowledge the reviewer must load: the identity that makes one quantity an upper bound on surprise, the two equivalent ways of splitting the action quantity into parts, the non-negativity of a divergence and what that constrains, the conjugate relationship that gives the learning counts a closed form, the requirement that an information term decay to nothing as counts grow, the rule that everything added together must be in the same units or be explicitly weighted, and a preference for breaking an idea in a two- or three-state world before trusting it in a large one.
