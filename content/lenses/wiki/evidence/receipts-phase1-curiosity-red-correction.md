...
lens_schema: 1
scope: wiki
key: evidence/receipts-phase1-curiosity-red-correction
corpus: evidence
source_sha256: 68fa9396a5604102
source_body_sha256: 68fa9396a5604102
source_title: P1 curiosity RED — magnitude correction (2026-07-11)
source_words: 510
...
The next section diagnoses how the headline was made. It picked one control body and reported the treatment as though its own remaining case were absent, which dropped both a second control case and that remainder. No committed receipt supports any of the withdrawn figures. A corrected statement is then written out in full, so there is something else to cite.

Finally a second, independent data point, recovered from an older snapshot, which points the same way with an even smaller ratio. Both are short single snapshots rather than a powered series, and the page says so, settling on the direction of the effect with a modest and noisy magnitude. That is the whole correction: same verdict, smaller number, and both versions kept where anyone can see them.
