...
<!--CLEAR-->
A receipt for one change, and the page argues it is the change separating being able to go live from being able to stay live. Every earlier receipt ended on the same standing item: nothing in the studio reaches a person who is not reading pixels, and a black picture kills no process and opens no dialog.
...
Then a crash the author introduced and found the hard way. A value that was present but null threw inside a timer tick and took the whole widget down. The observation is the sharp one: the surface that exists to say when something is wrong must never be the thing that dies. It was fixed at source across every numeric read and given a backstop so that a future fault is logged and the next tick retries. The closing fence names two things that are not verified: annunciation during a genuine broadcast, and that one of the three codes is an inference about the far end rather than a measurement of it.
