...
---
<!--PLAIN-->
A receipt for a piece of software that treats everything openable as a door, and records for each one how it opens, how it closes, and whether it is currently ready to do the opposite of whatever it is doing. Most of the drilled legs passed: a graceful close-all shut everything down cleanly, one key brought it all back, and a prediction the system made about itself came true on the timescale it named. One leg is honestly marked partial: a stubborn program ignores a polite request to close, and the receipt does not round that up. A fence holds in the middle of it: ask to close anything belonging to the colony and the answer comes back refused.

<!--CLEAR-->
A receipt with a mixed verdict, and the mix is deliberate. The drilled legs pass; the one leg that closes a particular program politely is recorded as partial rather than folded into the total.
...
There is a fence in the middle of it. Doors belonging to the colony do not open or close at all; the answer comes back refused, with a message saying this system only observes the bodies and never affects them, and that such changes belong to the operator through the approval queue. The drill shows that refusal as a real response rather than as a claim about one.
