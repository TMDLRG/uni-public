...
---
<!--PLAIN-->
This page describes how a codebase is tested. It is written for someone trying to judge whether the testing is any good, not just count it. It lists each suite and what it covers, then the parts that matter more than the list: a small property tester written in the project rather than pulled in from outside, a rule that time in tests is logical so that nothing can flake, a checklist mapping one test to each mandatory invariant, fuzzing aimed at leakage, long soak runs, and a stored reference episode that pins regressions.
