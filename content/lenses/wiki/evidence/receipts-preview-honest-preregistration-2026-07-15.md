...
<!--CLEAR-->
A pre-registration written before the change, with the cures recorded underneath as they landed, so the page reads as a promise followed by two records of what was run.
...
The honest primitive already exists in the code, and is quoted with corroboration from another recorded run the same session. A real frame is far larger than a solid black one, so rendering becomes a size threshold combined with a freshness window, while existence stays a separate bit. Black is never live.

The gates are given as a table, each with a pass condition and the result that would show it wrong. They cover the honesty split and the refresh rates for the different panels. The preview freezes once a scene is on air while the programme keeps refreshing, and an existing high-rate window still works. The new cadence stays within a stated cost budget, and the periodic test can run to completion without knocking the show off air.

A stated limit separates two meanings of one word. Here live means broadcast video, never a claim about life or awareness, and the collision with another use of the same token is named as out of scope and not made any worse.

The first cure lands the honesty split, described as the highest-leverage change and taken before any ergonomics. The recorded runs are what make it convincing: stale scenes read as no signal despite carrying plenty of bytes, which is the honest de-escalation, and the programme reads live again only after a fresh check. One label is changed from a bare live to a statement of what is actually known about it.
