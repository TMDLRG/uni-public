...
lens_schema: 1
scope: wiki
key: evidence/receipts-fanout-supervisor-honesty-2026-07-16
corpus: evidence
source_sha256: 9ec83a5d8add3997
source_body_sha256: 9ec83a5d8add3997
source_title: Receipt — the fan-out supervisor stops lying (and I retract an overclaim)
source_words: 1432
...
<!--CLEAR-->
A receipt whose first section is a retraction, in the author's own voice, of something said to the operator during a walkthrough. The sentence is quoted and then taken apart.
...
Why this matters more than a wrong sentence is the sharpest paragraph on the page. The prescribed order arms the destinations before the operator types the confirmation, so the procedure itself leaves a window in which perfectly good keys are accused of being rejected, which would send a live operator away mid-show to re-type working keys and chase nothing. What survives the retraction is stated too, because a different catch during the broadcast test was genuine.

Then the defects. A flap may now name the key only when the key could actually have been sent, using a fact already in hand about whether anything is publishing locally. The rehearsal shows the row staying red, because it genuinely is not pushing, while no longer accusing. The fence is explicit: this shows the honest message in the no-publisher case, and does not test the other branch against a real rejecting platform, which stays not verified.
