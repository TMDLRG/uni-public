...
<!--CLEAR-->
A receipt with a partial verdict, and the partial is the honest part of it. Three legs were run live and are recorded with their output; the fourth waits on something that had not happened yet, a real power cycle.

The build list comes first: a supervisor that watches the process and restarts it, a startup installer that leaves a marker recording when it was installed, an arbiter that checks the restart leg on its own and cannot pass falsely because it requires the boot to be later than that marker and the supervisor to have started after the boot, and an icon path that always tries to bring the chain up and falls back to a static triage page if nothing can run at all, so the door never dead-ends. One survival property is marked as inspected rather than executed, which is a weaker class, and it is labelled as such rather than counted with the rest.
