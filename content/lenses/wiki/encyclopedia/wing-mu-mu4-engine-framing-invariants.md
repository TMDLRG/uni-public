...
The framing section names three layers separated by two typed boundaries, with interoception as hardware self-signals and exteroception as system observations. It carries the chapter's load-bearing fence: exactness is a property of one interface, not of the assembled system. Writing that the whole is globally exact where only that interface is exact is exactly the calibration-up the constitution refuses.
