...
lens_schema: 1
scope: wiki
key: encyclopedia/wing-natura-na-08-cell-level-design
corpus: encyclopedia
source_sha256: 6f12b8f4c32b66dd
source_body_sha256: 6f12b8f4c32b66dd
source_title: NA-08 — Design down to the cell level: the cell as an engineered system
source_words: 5405
...
A cell gets treated here as an engineered artifact and written a working budget: what sets its size, what it spends, what its machines deliver under stated conditions, and where its sensing hits a physical floor. Two fences come before any of that. The biology is all somebody else's, measured elsewhere and quoted, so the chapter contributes no evidence to the programme's own results. And a closing list records what it does not claim — beginning with a flat refusal that a cell is aware of anything, and continuing with an equally flat refusal that building one is close, tractable, or on any roadmap. Nothing here is a construction plan. The physics the rest follows from is diffusion, where the wait goes as the square of the distance, so double the cell and quadruple it. Read down the ladder of distances and the last row is load-bearing: a long nerve cell cannot use diffusion for transport at all, not merely inefficiently. So it does not, and a motor walks the cargo instead. A second ceiling is geometric, since volume outgrows the surface that feeds it.
