...
The verdict is licensed by the lower bound of a multi-seed interval, not by the point estimate, and that bound clears the bar that was set before the run, comfortably. The result is checked by three gates and earned the ledger's first genuinely validator-derived reproduction flag, derived from several distinct seeds and a real, non-degenerate interval rather than written in by hand. One fence about where a figure came from travels with it, since a separate replication row records a different margin and interval, and the two are distinct measurements that must never be quoted for each other.
