...
lens_schema: 1
scope: wiki
key: encyclopedia/wing-s-s-l6-perception-world-c
corpus: encyclopedia
source_sha256: 4888e7bdc4951ccd
source_body_sha256: 4888e7bdc4951ccd
source_title: S-L6 - Perception: World C, the one citable empirical PASS
source_words: 1848
...
One result in the whole program is ready to be put in front of an outside examiner, and this chapter is about that result. A reader built only from counts, with no gradient step anywhere in it, beats a tuned baseline on sealed held-out real text. It is exactly one thing: a count-baseline win. It is not active inference shown, not comprehension, not talking, and not a win over large language models, since by a chosen design trade the same program sits behind gradient-trained models on character perplexity. The program around it stays a developmental active-inference simulation, a bounded peek at a toy world and never a person. The pass is real, its ceiling is fixed, and the chapter prints the two negatives that bound it on either side: of many structural designs tried only the cache wins, and the gain turned out diffuse, so there was nothing left for a controller to gate.

<!--CLEAR-->

The reader is built from exact count statistics and a multi-level recency cache. Its only learning is conjugate count addition, with no automatic differentiation, no gradient step and no trained network. It is evaluated against a tuned smoothing baseline on sealed, held-out real text, and the baseline is properly tuned rather than a strawman, because the discipline requires a tuned opponent before any margin is claimed.

The verdict is licensed by the lower bound of a multi-seed interval, not by the point estimate, and that bound clears the pre-registered bar comfortably. The result is checked by three gates and earned the ledger's first genuinely validator-derived reproduction flag, derived from several distinct seeds and a real, non-degenerate interval rather than written in by hand. One provenance fence travels with the figures, since a separate replication row records a different margin and interval, and the two are distinct measurements that must never be quoted for each other.

A family of supporting held results corroborates the flagship, each bounded the same way. One of them matters most for interpretation, because it beats an unbounded backbone and so closes the common deflationary reading that the baseline simply had too short a window. A recency ablation removes a measurable part of the win, which is what a true ablation should do.
