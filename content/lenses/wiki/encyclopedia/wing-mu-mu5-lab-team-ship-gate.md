...
Further sections cover a receipt loop, in which each burst of work closes with a falsifiable receipt, and a documentation discipline whose first implementation was wrong and was caught. Then comes a durable runner, built on the principle that silence is not success, and that a status chip reading as running over a dead process is a falsified inference. Last is a bound on multi-agent fan-out, derived from a burn in which most of the agents died.
