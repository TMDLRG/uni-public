...
lens_schema: 1
scope: wiki
key: encyclopedia/wing-natura-na-10-the-scale-ladder-and-gradients
corpus: encyclopedia
source_sha256: 6e7a9fc0d6b24f26
source_body_sha256: 6e7a9fc0d6b24f26
source_title: NA-10 — The scale ladder: quark to galaxy, and the gradients between
source_words: 7410
...
Cited, not measured — nothing on this page raises a rung of the programme's own ledger, because a nature citation is never a UNI gate, and the page says so before its first number. What it lays out is the wing's spine: a rung-by-rung table from something very small to something very large, and, as the real payload, a rule for knowing when a number fitted on one rung stops meaning anything on the next. The picture to drop is the ladder as a smooth continuum with things sprinkled along it. It is a set of regimes with boundaries between them, and the boundaries are where the engineering value sits. Stay inside a regime and one force dominates, a group or two governs, and a fitted power law behaves. Cross a boundary and the dominant force changes identity, so any exponent carried across is a category error wearing a decimal point. The chapter classes itself as modelled for exactly that reason. Read the boundary column down the table and it does not rise with size. Some very large things have no boundary and some very small ones do. Size is not what makes one.

<!--CLEAR-->

The table places each rung with a characteristic length, a characteristic time belonging to that rung's own dynamics, a characteristic energy, the dominant force or constraint, the governing dimensionless groups, and whether a boundary of the kind an earlier chapter defines is identifiable there.
...
The chapter is classed as modelled overall, and its closing fence is a list of things it does not claim about itself. Not that the ladder is a progression, a hierarchy of value or a direction, since it is an ordering by length and one measure of specific power runs the other way along it. Not that emergence is mysterious, and not that it is trivial, both readings being defects. Not that a framework of major transitions is beyond debate. Not that one published exchange about curvature has a winner, since both turns are carried and neither is the verdict. Not that its chosen rungs are the only defensible carving. And the standing one: no citation above raises any rung, because the vocabulary for nature's regularities and the vocabulary for the programme's build status describe different kinds of claim and never merge.
