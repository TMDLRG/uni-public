...
lens_schema: 1
scope: wiki
key: encyclopedia/wing-mu-mu3-negatives-bounds-two-tier
corpus: encyclopedia
source_sha256: 11a132299b48b373
source_body_sha256: 11a132299b48b373
source_title: mu3 - Negatives, bounds and the two-tier split
source_words: 1753
...
Losing well is the subject of this chapter, and it asserts no capability of its own. It sets out how the program handles a result that did not work: why a negative is kept as content instead of quietly dropped, what a finding has to clear before anyone may rest on it as a bound, and where the line falls between the one track that could face an outside bar and a second, synthetic track that an audit reduced to artifact and diagnostic. The program on both sides of that line is a developmental active-inference simulation, a bounded peek at a toy world and never a person. The governing rule is that a partial outcome measures an incomplete design and gives nobody permission to stop, so only two resting places are legitimate: something that works, or a published, exhausted, falsifiable bound. One fence here is absolute. The synthetic track may never be inflated into a capability.

<!--CLEAR-->

The chapter starts from a no-exit discipline. Under it, exiting on a partial or a negative without a published exhausted bound is the violation, as is treating an undischarged park as if it were closed. The program currently carries one such open obligation, since a frontier is parked but its formal sign-off was drafted, relayed and not yet captured.

A negative does not become a published bound the moment it appears. A registered bar sets what is required first: several structurally distinct held negatives, each changing at least two of a named set of axes, with the mundane causes falsified before any deeper negative may be called. The worked example carries both halves of a sensorimotor result, one design held positive and a structurally distinct one held negative with its whole interval below zero, and records the count toward exhaustion, so no bound is owed and none is claimed. One clean negative is a result, not a bound.
...
Two further rows protect the integrity of any published negative: one naming the test-rigor smells that fake a verdict, including a length confound under which an agent that dies fast can look better, and one forbidding stacked changes that make an outcome unattributable.
