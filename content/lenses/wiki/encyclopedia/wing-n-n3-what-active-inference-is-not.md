...
lens_schema: 1
scope: wiki
key: encyclopedia/wing-n-n3-what-active-inference-is-not
corpus: encyclopedia
source_sha256: fa73060ac3984ba6
source_body_sha256: fa73060ac3984ba6
source_title: N3 - What active inference is NOT (the meaning of the fences)
source_words: 1811
...
Every other chapter in this wing says what the program has shown. This one says what it has not, and why each fence sits exactly where it does. It is the not-claimed chapter for the whole wing, and it asserts only boundaries. The thing being fenced is a simulation — a toy world, and not a person. Four lines are drawn. A count-baseline win on text set aside and scored once is a better predictor; it is not a reader that comprehends. A passed battery of functional self-model tests is a function, not a feeling, and phenomenal sentience is disclaimed rather than tested, so no falsifier is offered for it. A machine reading its own telemetry through a categorical channel is instrumented, not aware. And active inference here is a textbook lens for organising the work, never a demonstrated loop. Each line is drawn by the published negative that defines it.

<!--CLEAR-->

The chapter states four boundaries, each with the exact result it fences and the negative that travels with it.
